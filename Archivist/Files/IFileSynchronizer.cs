...
internal sealed class FileSynchronizerService : IFileSynchronizer
{
        ...
        ChatMetadata sourceMetadata)
    {
        ...
        {
            _logger.Error(
                $"Не удалось прочитать метаданные: {destinationFilePath}",
                exception);
        ...
        {
            _logger.Error(
                $"Не удалось прочитать файл: {destinationFilePath}",
                exception);
            AddDestinationErrorIfNew(destinationFilePath, exception);
            return FileOperationStatus.Added;
        }
    ...
    private void AddDestinationErrorIfNew(
        string path,
        Exception exception)
    {
