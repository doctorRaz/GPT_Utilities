...
namespace dRz.GPT_Utilities.Archivist.Tests.Files
{
    public sealed class FileSynchronizerCopyTests
    {
        ...
        private const string ConversationB = "https://chatgpt.com/c/bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb";

        [Test] public void CopyIfNewer_AddsFile_WhenDestinationDoesNotExist()
        {
        ...
        [Test] public void CopyIfNewer_ReplacesFile_WhenSourceIsNewer()
        {
            using TempDirectory temp = new();
        ...
        [Test] public void CopyIfNewer_SkipsFile_WhenSourceIsOlderOrEqual()
        {
            using TempDirectory temp = new();
        ...
        [Test] public void CopyIfNewer_AddsUniqueFile_WhenConversationIdsDiffer()
        {
            using TempDirectory temp = new();
        ...
        [Test] public void CopyIfNewer_ReimportOfSameConversation_UpdatesExistingUniqueFile()
        {
            using TempDirectory temp = new();
            string destination = MarkdownFactory.Write(temp.Combine("dst", "Chat.md"), CreateTime, CreateTime.AddHours(1), ConversationA, "first");
        ...
        [Test] public void CopyIfNewer_ReplacesMatchingUniqueFile_InsteadOfAddingDuplicate()
        {
            using TempDirectory temp = new();
            ...
            string replacement = temp.Combine("dst", "Chat (1).md");
            Assert.That(result.Status, Is.EqualTo(FileOperationStatus.Updated));
            Assert.That(File.Exists(matchingDestination), Is.False);
            Assert.That(File.Exists(replacement), Is.True);
            Assert.That(File.ReadAllText(replacement), Does.Contain("new second"));
            Assert.That(temp.Combine("dst", "Chat (2).md"), Does.Not.Exist);
        }

        [Test] public void CopyIfNewer_SkipsNewestMatchingUniqueFile_WhenSeveralExist()
        {
            using TempDirectory temp = new();
            _ = MarkdownFactory.Write(temp.Combine("dst", "Chat.md"), CreateTime, CreateTime.AddHours(1), ConversationA, "first");
        ...
        [Test] public void CopyIfNewer_SkipsMatchingUniqueFile_WhenItIsNewer()
        {
            using TempDirectory temp = new();
            _ = MarkdownFactory.Write(temp.Combine("dst", "Chat.md"), CreateTime, CreateTime.AddHours(1), ConversationA, "first");
        ...
        [Test] public void CopyIfNewer_AddsUniqueFile_WhenBothConversationIdsAreMissing()
        {
            using TempDirectory temp = new();
        ...
        [Test] public void CopyIfNewer_AddsUniqueFile_WhenOneConversationIdIsMissing()
        {
            using TempDirectory temp = new();
        ...
        [Test] public void CopyIfNewer_SetsLastWriteTimeFromUpdateTime()
        {
            using TempDirectory temp = new();
