...
namespace ConvovizRenamer
{
    ...
    /// первого элемента YAML-свойства <c>aliases</c> и
    /// обновляет соответствующий <c>_index.md</c>.
    /// </summary>
    /// <remarks>
    ...
    /// </code>
    ///
    /// При этом ссылки в _index.md автоматически исправляются.
    /// </remarks>
    public sealed class MarkdownRenamer
    {
    //todo забыл переименовать title
        /// <summary>
            ...
            RenameResult result)
        {
            ...
            // Первый проход.
            //
            // Только определяем, какие файлы необходимо переименовать.
            // Файлы пока НЕ перемещаем.
            // ---------------------------------------------------------
            ...
            foreach (var pair in renameMap)
            {
                try
                {
                    ...
                    performedRenames[pair.Key] = pair.Value;

                    result.Renamed++;

                    Console.WriteLine(
                        $"[OK] {Path.GetFileName(pair.Key)}" +
            ...
            // ---------------------------------------------------------
            // Обновляем _index.md только после успешного переименования.
            // ---------------------------------------------------------
        ...
        /// <summary>
        /// Получает все каталоги дерева, включая корневой.
        /// </summary>
        ...
        /// <summary>
        /// Обновляет ссылки в _index.md согласно выполненным
        /// переименованиям.
        /// </summary>
            ...
            IReadOnlyDictionary<string, string> renameMap)
        {
            ...
            foreach (var pair in renameMap)
            {
                ...
                string oldLink = UrlEncodeFileName(oldFileName);
                string newLink = UrlEncodeFileName(newFileName);
        ...
        /// <summary>
        /// Кодирует имя файла для использования в Markdown-ссылке.
        /// </summary>
        /// <remarks>
        /// Например:
        ///
        ...
        /// </code>
        ///
        /// При этом Unicode-символы сохраняются.
        /// </remarks>
        private static string UrlEncodeFileName(string fileName)
        {
        ...
        /// <summary>
        /// Читает первый элемент YAML-списка aliases.
        /// </summary>
        ...
        /// </summary>
        /// <remarks>
        /// Не используется ASCII-фильтрация.
        ///
        ...
        private static string SanitizeFileName(string name)
        {
            ...
            // Windows запрещает пробелы и точки в конце имени.
            name = name.TrimEnd(' ', '.');
    ...
    public sealed class RenameResult
    {
        ...
        /// <summary>
        /// Количество обновлённых _index.md.
        /// </summary>
